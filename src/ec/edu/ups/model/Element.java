...
public class Element {
    ...
    public Element(int pointX, int pointY, BufferedImage image,
	    boolean selected, char option, String facesPath, int sizeFaces,
	    int nFaces, int pointXF, int pointYF) {
	super();
	this.pointX = pointX;
	this.pointY = pointY;
	this.image = image;
	...
	this.edge = new Rectangle(this.pointX + width / 2,
		this.pointY + height / 2, 1, 1);
...
//	System.out.println(edge.toString());
	...
	edge.setRect(pointX + width / 2, pointY + height / 2, 1, 1);

	g.drawImage(image, pointX, pointY, width, height, null);

	g.drawImage(faces.getCurrentFace(), pointX + pointXF, pointY + pointYF,
		width / 2, height / 2, null);

//	g.drawRect((int) edge.getX(), (int) edge.getY(), (int) edge.getWidth(),
//		(int) edge.getHeight());
    }
