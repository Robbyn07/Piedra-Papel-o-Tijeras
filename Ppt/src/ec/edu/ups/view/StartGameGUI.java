...
public class StartGameGUI extends JPanel implements ActionListener {
	...
	JPanel p1 = new JPanel() {
		...
		ImageIcon imagen = new ImageIcon(
			getClass().getResource(Constants.BACKGROUND_PATH));

		g.drawImage(imagen.getImage(), 0, 0, tamano.width,
			tamano.height, null);

		g.setColor(new Color(30, 30, 30, 70));
		g.fillRect(0, (int) getSize().getHeight() - 100,
	...
	try {
		...
		if ((play1.length() > 10) || (play2.length() > 10)) {
		    JOptionPane.showMessageDialog(null, "Max. 10 Caracteres.",
			    "ERROR", JOptionPane.ERROR_MESSAGE);
		} else {
		    if ((play1.length() == 0) || (play2.length() == 0)) {
			JOptionPane.showMessageDialog(null,
				"Ingrese el nombre para el jugador.", "ERROR",
			...
			if (number <= 0) {
			    ...
			    this.startGameState.getMainMager()
				    .setCurrentPanel(1);
			    this.startGameState.getMainMager().getStateManager()
				    .getGameRuleManager()
				    .setRoundNumber(number);
			    this.startGameState.getMainMager().getStateManager()
				    .getGameRuleManager().getPlayers()[0]
					    .setName(player1.getText()
						    .toUpperCase());
			    this.startGameState.getMainMager().getStateManager()
				    .getGameRuleManager().getPlayers()[1]
					    .setName(player2.getText()
						    .toUpperCase());
			}
		    }
		}
