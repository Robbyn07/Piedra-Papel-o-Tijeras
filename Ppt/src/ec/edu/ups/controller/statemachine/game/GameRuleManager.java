...
public class GameRuleManager implements GameState {
	...
	public void update(StateManager stateManager) {
		...
		if (status) {
			...
			if (gameWinner == null) {
				String winner = mainManager.getStateManager().getGameRuleManager().getPlayers()[0].getName()
						+ mainManager.getStateManager().getGameRuleManager().getPlayers()[1].getName();
				this.mainManager.getStateManager().getGameWinnerManager().getEndGameGUI().setGameWinnet(winner);
			} else {
				this.mainManager.getStateManager().getGameWinnerManager().getEndGameGUI()
						.setGameWinnet(gameWinner.getName());
			}
